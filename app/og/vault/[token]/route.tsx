...
/**
 * The unfurl card for a shared vault.
 *
 ...
 *
 * Data comes from shared_vault(), the same token-gated function the page
...
async function fetchVault(token: string): Promise<SharedVault | null> {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  if (!url || !key) return null;

  const res = await fetch(`${url}/rest/v1/rpc/shared_vault`, {
    method: "POST",
    headers: {
      apikey: key,
  ...
  const data = (await res.json()) as SharedVault | null;
...
export async function GET(
  ...
  const data = await fetchVault(params.token);
