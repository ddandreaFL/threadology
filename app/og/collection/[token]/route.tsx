...
/**
 * The unfurl card for a shared collection.
 *
 ...
 *
 * Data comes from shared_collection(), the same token-gated function the page
...
async function fetchCollection(token: string): Promise<SharedCollection | null> {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  if (!url || !key) return null;

  const res = await fetch(`${url}/rest/v1/rpc/shared_collection`, {
    method: "POST",
    headers: {
      apikey: key,
  ...
  const data = (await res.json()) as SharedCollection | null;
...
export async function GET(
  ...
  const data = await fetchCollection(params.token);
